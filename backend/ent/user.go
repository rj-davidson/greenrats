...
type User struct {
	...
	// WorkosID holds the value of the "workos_id" field.
...
func (e UserEdges) PicksOrErr() ([]*Pick, error) {
	if e.loadedTypes[0] {
		return e.Picks, nil
	}
...
// LeagueMembershipsOrErr returns the LeagueMemberships value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) LeagueMembershipsOrErr() ([]*LeagueMembership, error) {
	if e.loadedTypes[1] {
	...
	for i := range columns {
		...
		case user.FieldWorkosID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field workos_id", values[i])
			} else if value.Valid {
				_m.WorkosID = value.String
			}
...
// QueryPicks queries the "picks" edge of the User entity.
func (_m *User) QueryPicks() *PickQuery {
	return NewUserClient(_m.config).QueryPicks(_m)
}
...
func (_m *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("workos_id=")
	builder.WriteString(_m.WorkosID)
	builder.WriteString(", ")
