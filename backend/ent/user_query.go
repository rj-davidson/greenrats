...
func (_q *UserQuery) QueryPicks() *PickQuery {
...
// QueryLeagueMemberships chains the current query on the "league_memberships" edge.
func (_q *UserQuery) QueryLeagueMemberships() *LeagueMembershipQuery {
...
func (_q *UserQuery) QueryEmailReminders() *EmailReminderQuery {
...
func (_q *UserQuery) WithPicks(opts ...func(*PickQuery)) *UserQuery {
	query := (&PickClient{config: _q.config}).Query()
...
// WithLeagueMemberships tells the query-builder to eager-load the nodes that are connected to
// the "league_memberships" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithLeagueMemberships(opts ...func(*LeagueMembershipQuery)) *UserQuery {
	query := (&LeagueMembershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithEmailReminders(opts ...func(*EmailReminderQuery)) *UserQuery {
	query := (&EmailReminderClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withPicks; query != nil {
		if err := _q.loadPicks(ctx, query, nodes,
			func(n *User) { n.Edges.Picks = []*Pick{} },
			func(n *User, e *Pick) { n.Edges.Picks = append(n.Edges.Picks, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withLeagueMemberships; query != nil {
	...
	if query := _q.withEmailReminders; query != nil {
		if err := _q.loadEmailReminders(ctx, query, nodes,
			func(n *User) { n.Edges.EmailReminders = []*EmailReminder{} },
			func(n *User, e *EmailReminder) { n.Edges.EmailReminders = append(n.Edges.EmailReminders, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadPicks(ctx context.Context, query *PickQuery, nodes []*User, init func(*User), assign func(*User, *Pick)) error {
	...
	query.Where(predicate.Pick(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PicksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_picks
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_picks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_picks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadLeagueMemberships(ctx context.Context, query *LeagueMembershipQuery, nodes []*User, init func(*User), assign func(*User, *LeagueMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.LeagueMembership(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.LeagueMembershipsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_league_memberships
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_league_memberships" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadEmailReminders(ctx context.Context, query *EmailReminderQuery, nodes []*User, init func(*User), assign func(*User, *EmailReminder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.EmailReminder(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EmailRemindersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_email_reminders
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_email_reminders" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_email_reminders" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
